...
class TournamentCLI {
  ...
  async playAnonymously() {
    ...
    try {
      ...
      console.log(
        'You can still try to use tournament features, but they may not work properly.'
      )
    }
      ...
      if (Array.isArray(response)) {
        if (response.length === 0) {
          ...
          response.forEach((tournament, index) => {
            const status = tournament.found
              ? 'You are in this tournament'
              : 'Available to join'
            console.log(`${index + 1}. ${tournament.creator}'s Tournament`)
            console.log(`   ${status}`)
            console.log(
              `   Players: ${tournament.playersIn}/${tournament.playersExpected}`
            )
        ...
        console.log(
          `   Players: ${response.playersIn}/${response.playersExpected}`
        )
  ...
  async tryStartMatch() {
    ...
    if (gameId) {
      ...
      console.log(
        "   Either the tournament hasn't started or you don't have a match to play."
      )
  ...
  handleGameMessage(message) {
    ...
    if (typeof message === 'object') {
      ...
      if (message.type === 'role') {
        ...
        console.log(
          `You are playing as: ${
            this.playerRole === 'left'
              ? 'Left Player'
              : this.playerRole === 'right'
              ? 'Right Player'
              : 'Spectator'
          }`
        )
      ...
      if (message.type === 'message' && message.message) {
        ...
        if (msg.includes('waiting_for_readiness')) {
          ...
          console.log(`${msg}`)
        } else if (msg.includes('left_error')) {
          console.log('Walkover! Left player left the game. Right player wins!')
        } else if (msg.includes('right_error')) {
          console.log('Walkover! Right player left the game. Left player wins!')
        }
        return
      }
    }
    ...
    stdin.on('data', (key) => {
      ...
      if (this.wsConnection) {
        ...
        if (key === 'r' || key === 'R') {
          if (this.readySent) {
            console.log(
              'READY signal already sent. Waiting for other player...'
            )
            return
          }
  ...
  async establishWebSocketConnection() {
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        if (
          this.invitationWs &&
          this.invitationWs.readyState === WebSocket.CONNECTING
        ) {
