...
export function updateGameState(gameState, ballSpeed) {
  ...
  if (
    gameState.ball.y + ballRadius > canvasHeight &&
  ...
  if (gameState.ball.x < 0 || gameState.ball.x > canvasWidth) {
    if (gameState.ball.x < 0) gameState.score.right += 1;
    if (gameState.ball.x > canvasWidth) gameState.score.left += 1;
    if (
      gameState.score.left >= GAME_CONFIG.NUMBER_OF_ROUNDS ||
      gameState.score.right >= GAME_CONFIG.NUMBER_OF_ROUNDS
    ) {
...
export function resetGameStatus(game, rematch = true) {
  ...
  game.readyL = false;
  game.readyR = false;
  if (rematch) broadcastMessage(game.clients, 'rematch');
  else broadcastMessage(game.clients, 'reset');
...
function paddleBounce(gameState, ballSpeed, side) {
  if (side === 'left') {
    ...
    const relativeIntersectY =
      (gameState.ball.y - paddleCenter) / (extendedPaddleHeight / 2);
    const bounceAngle = relativeIntersectY * ((Math.PI * 75) / 180);
    const speed = Math.sqrt(
      ballSpeed.ballSpeedX * ballSpeed.ballSpeedX +
        ballSpeed.ballSpeedY * ballSpeed.ballSpeedY,
    );
