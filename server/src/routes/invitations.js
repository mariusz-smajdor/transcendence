	...
	fastify.get('/invitations', { websocket: true }, (connection, req) => {
		...
		connection.on('message', (message) => {
			...
			if (data.type === 'auth') {
				...
				connection.send(JSON.stringify({ type: 'cookies', token: data.token, sessionId }));
		...
		connection.on('close', () => {
			...
			if(userId){
				if(clients.get(userId) === connection)
...
function scanWs(key){
	if(clients.get(key))
		return true;
	else if(notAuthenticated.get(key))
		return true;
	return false;
}
...
export function closeOldWs(key){
	...
	if (!oldWs)
		oldWs = notAuthenticated.get(key);
	wsMessage("Connection closed! Please close this tab and continue in your new one.", oldWs);
	oldWs.close();
}
